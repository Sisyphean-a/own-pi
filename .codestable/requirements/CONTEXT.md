...
目前没有单独的工作区业务领域；可选扩展依赖是跨包架构契约，权威记录见 [可选扩展依赖](../architecture/shared/optional-extension-dependencies.md)。`package:pi-lsp-feedback`、`package:pi-quick-panel`、`package:pi-lean-tool-display`、`package:pi-optimization`、`package:pi-wait` 和 `package:pi-advisor` 分别是各自领域的实现边界。
