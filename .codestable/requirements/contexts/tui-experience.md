...
- TUI 编辑器中的 `Ctrl+L` 与 `/quick-panel` 都打开快捷面板；非 TUI 模式不创建面板，并明确提示该功能不可用。快捷面板编辑器通过 `CustomEditor` 的 `embedWorkingStatus: true` 接管内置工作状态：working、重试、压缩等状态显示在输入框上边框内，不另起独立状态行；状态文本后跟一个无文案的呼吸图标，只在模型生成（思考）期间出现（`turn_start` 起、`tool_execution_start`/`turn_end`/`agent_settled` 止），颜色取 `editor.borderColor`（当前思考等级色，与 Pi 的 spinner、输入框边框同源），帧由渲染时刻推导并随 Pi 自身的帧重绘推进，不持有本扩展的定时器。
...
- footer 优先单行显示仓库/分支、统计、上下文和当前模型；空间不足时才拆行，扩展状态保持独立行。实时 Thinking 标记不在 footer，而是作为图标显示在输入框上边框的状态文本之后（仅模型生成期间）。
