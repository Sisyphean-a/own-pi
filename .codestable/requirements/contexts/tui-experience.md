...
- 工具标题和结果采用紧凑摘要，但必须保留路径、错误、行数和 diff；折叠态每个可分组工具行最多保留一行正文，多行命令只显示第一行并分别标出命令行数与输出行数，流式参数和输出更新不得改变该工具行高度；裁剪只在折叠态生效，`Ctrl+O` 或点击展开后输出完整结果；连续可见工具调用合并间距，`edit` 和 `write` 保持 Pi 原生渲染与独立边界。
