...
- 每次 `write`/`edit` 后的诊断只同步等待一个固定预算：预算内完成时按上述规则在本轮 `turn_end` 反馈；超出预算时转由后台收尾，结果在下一轮 `turn_end` 经同一去重与稳定门禁反馈，不再拖住 Pi 的工具调用路径。
- 同一 `<server id>:<root>` 同时只保留一个在途启动：并发检查复用同一客户端，不得让后一个启动覆盖前一个或使其失去关闭路径。诊断服务关闭后不再新建语言服务器客户端；关闭期间已在启动中的客户端及 Vue TypeScript sidecar 一旦就绪立即终止，`close()` 等待在途启动落定，不得留下无人持有的语言服务器进程。
