...
- **实时思考标记**：模型回合进行时显示在 footer 摘要行的统计数值与模型之间、进入工具执行或回合结束后隐藏的活动状态。
...
- 实时思考标记只在模型回合进行时显示：宽屏在 footer 摘要行的统计数值与右对齐模型之间显示 `● Thinking`，尾部圆点每秒按 0、1、2、3 个循环并预留三格避免布局抖动，窄屏缩为 `●`；工具开始执行、回合结束、代理稳定或会话关闭时停止计时并隐藏。安装自定义 footer 成功后，内置 working 行隐藏，避免重复状态。
