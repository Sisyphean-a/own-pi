...
- **视觉减负**：在保留行动信息的前提下减少低价值信息和界面噪音，使 Pi 的 TUI 更美观、简洁、优雅。
...
- **provider usage 状态**：当前 `openai-codex` 模型 5 小时/周窗口，或 `opencode-go` 模型 5 小时/周/月窗口的剩余百分比状态栏信息。

## 稳定规则

- 视觉减负优先减少低价值信息和界面噪音，但必须保留路径、错误、行数和 diff 等行动信息，不改变消息语义。
...
- Codex usage 将接口 `rate_limit.primary_window` 映射为 5 小时额度、`rate_limit.secondary_window` 映射为周额度；OpenCode Go usage 将官方 `usage.rolling`、`usage.weekly`、`usage.monthly` 映射为 5 小时、周和月额度。两者都显示剩余百分比；请求只在当前 provider 为 `openai-codex` 或 `opencode-go` 且能取得对应认证信息时执行；失败、超时、非目标模型或 stale context 不得抛出未处理异常，也不得继续注册旧轮询。
