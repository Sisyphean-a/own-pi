...
- 实时思考标记只在模型回合进行时显示：宽屏在 footer 摘要行的统计数值与右对齐模型之间显示 `✻ Thinking…`，图标按 `✻`、`✢`、`✶`、`·` 的顺序每 250ms 切换一帧并循环一秒，分别使用 `thinkingLow`、`thinkingMedium`、`thinkingHigh`、`thinkingXhigh` 主题色；窄屏只保留同一动态图标，固定一个字形单元避免布局抖动。工具开始执行、回合结束、代理稳定或会话关闭时停止计时并隐藏。安装自定义 footer 成功后，内置 working 行隐藏，避免重复状态。
