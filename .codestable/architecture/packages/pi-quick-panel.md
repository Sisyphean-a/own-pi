...
`pi-quick-panel` 是一个以便捷为核心的私有 Pi TUI 快捷面板：它提供技能、模型、思考等级和模型组合的统一选择、查看与筛选入口，让用户更容易判断技能效果并快速应用技能或切换模型；当前模型为 Codex 官方账号时，面板页脚还异步显示 5 小时与周限额及其重置信息；同时把编辑器中的 `/skill:<name>` 指令展开为技能正文。
...
- `src/quick-panel-ui.ts` 只负责面板状态、筛选、Tab 切换、列表渲染、页脚用量布局和选择结果；`src/codex-usage.ts` 负责 Codex 官方 usage 请求、响应解析和重置信息格式化；`src/quick-panel-editor.ts` 只负责编辑器快捷键接入。
...
- Codex 用量：`packages/pi-quick-panel/src/codex-usage.ts`
...
- Codex 用量回归：`packages/pi-quick-panel/test/codex-usage.test.ts`
