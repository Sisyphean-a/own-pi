...
`pi-tui-enhancements` 是 Pi 的 TUI 体验增强包：把快捷面板、紧凑显示与上下文查看统一分发，减少多个独立包同时安装时重复注册快捷键、输入处理器、显示补丁和 provider usage 的风险；三个功能域仍保持独立装配。
...
- `src/display/` 只拥有消息、工具和 footer 的显示补丁与生命周期；`src/panel/` 只拥有面板、技能、组合和编辑器快捷键；`src/context/` 只拥有 `/context` 弹窗、标签页与 token 分类。工具显示策略集中在 `src/display/tool-policy.ts`，渲染补丁只消费该策略。
...
- 面板选择、上下文查看与显示补丁不共享可变状态；三者只通过 Pi 提供的模型、UI 和生命周期接口协作。
