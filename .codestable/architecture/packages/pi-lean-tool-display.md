...
`pi-lean-tool-display` 是一个以视觉减负为目标的 Pi TUI 显示扩展：它减少低价值信息和界面噪音，让用户无需接收大量冗余内容，同时在保留行动信息的前提下提供更美观、简洁、优雅的视觉体验；实现上通过集中管理的 prototype patch 提供紧凑工具调用、用户消息和思考内容渲染，通过自定义 footer 提供自适应紧凑状态布局，并显示 Codex 5 小时和周窗口的剩余百分比。
