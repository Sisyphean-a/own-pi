...
- `DiagnosticService` 接收已解析的服务器清单，负责文件分类、根目录解析、客户端复用和标准化结果；客户端缓存键为 `<server id>:<root>`。Vue 客户端可选地启动包内 TypeScript language server sidecar，供 Volar 转发 `tsserver/request`，并将同一快照的 `semanticDiagnosticsSync` 与 SFC 诊断合并；sidecar 启动、预热或请求失败时保留 Vue parser-only 路径。
