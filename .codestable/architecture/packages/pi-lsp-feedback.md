...
`pi-lsp-feedback` 是私有 Pi 扩展包。在 `write` 或 `edit` 工具结果成功后，它报告 LSP 诊断信息；它按语言服务器和工作区根目录启动并复用客户端，再把需要报告的诊断发送到代理的下一轮。
...
- 运行时依赖：`vscode-jsonrpc`；语言服务器可执行文件是预先安装的外部工具，扩展不会管理其安装。
...
- `servers.js` 是内置语言映射、根目录标记、本地命令查找及允许的受信任覆盖项的来源。
...
- 服务器选择与根目录：`packages/pi-lsp-feedback/src/servers.js`（`BUILTIN_SERVERS`、`findWorkspaceRoot`、`mergeServerOverrides`）。
